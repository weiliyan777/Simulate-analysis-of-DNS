...
def find_in_file(domain,data,rd):
    ...
    flag = 0
    jsons = []
    fopen = open("D:\VS code\.vscode\DNS\soc_test_com.txt",'r')
    for line in fopen:
        if line != '\n':
...
def socket_service():
    try:
        ...
        print(msg)
        sys.exit(1)
    print('here is dns.test.com-servise and waiting msg...')
    ...
    while 1:
        ...
        if flag :
            ...
            dolen = len(domains)
            topdomain = domains[dolen-3] +'.'+domains[dolen-2] +'.'+ domains[dolen-1]
            nsdomain=[]
            ##从文件中取出匹配顶级域名的信息
            fopen = open("D:\VS code\.vscode\DNS\soc_testcom_ns.txt")
            for line in fopen:
                if line != '\n':
                    jsObj = json.loads(line)
