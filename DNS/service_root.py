...
def find_in_file(domain,data,rd):
    ...
    flag = 0
    jsons = []
    fopen = open("D:\VS code\.vscode\DNS\soc_root.txt",'r')
    for line in fopen:
        if line != '\n':
...
def dnsrespose_no(data,rd,answer = 0,author = 0,addition = 0):  #生成空响应报文
    #根据查询类型分类 
    if rd :
        flag1=129
...
def dnsrespose_addi(reqdata,nsdomain,name_offset): ###生成addition段内容的响应报文
...
def dnsrespose_auth(data,nsdomain,rd): ###生成Authority段内容的响应报文
...
def socket_service():
    try:
        ...
        print(msg)
        sys.exit(1)
    print('here is root-servise and waiting msg...')
    ...
    while 1:
        ...
        if flag :
            ...
            nsdomain=[]
            ##从文件中取出匹配顶级域名的信息
            fopen = open("D:\VS code\.vscode\DNS\soc_root_ns.txt")
            for line in fopen:
                if line != '\n':
                    jsObj = json.loads(line)
            ...
            if  len_nsdomain > 0 :
                ...
                s.sendto(respdata,addr)
                print('已对servise',addr,'发送响应报文（无结果）')
        print('here is root-servise and waiting msg...')
    s.close()
